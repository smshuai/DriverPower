...
# singleTumor
# CADD
combined = read.table('./lncrna.ncrna//sig.only/combined.lncrna.ncrna.tsv', header=T, stringsAsFactors = F)
...
file_names = grep('cadd', list.files('./lncrna.ncrna/driverpower/', full.names=TRUE), value=TRUE)
...
func_cutoff = 70
dp = generate_dp(file_names, func_cutoff, ntest = 5580)
# fix name
...
dp$tumor[dp$tumor == 'Breast-AdenoCa'] = 'Breast-AdenoCA'
write.table(dp, './lncrna.ncrna/sig.only/driverpower.lncrna.ncrna.cadd80.singleTumor.tsv', sep = '\t', quote = F, row.names = F)
# combined single
...
heatmap.single = ggplot(dat.single, aes(x=Var2, y=Var1, fill=Freq, label=Freq)) + geom_tile() + geom_text(color='white') + theme_Publication() +
  ...
  guides(fill=F)
ggsave('../figures/plot/num.sig.singleType.lncrna.ncrna.heatmap.png', heatmap.single, height = 10, width = 8)
...
ggsave('../figures/plot/nsup.singleType.lncrna.ncrna.barplot.png', bar.single, height = 6, width = 6)

res = internal_benchmark(combined.single, cutoff.sup = 3, cutoff.cv = 2,
                         fig.path = '')

# EIGEN 93
file_names = grep('eigen', list.files('./lncrna.ncrna//driverpower/', full.names=TRUE), value=TRUE)
keep = tolower(sapply(file_names, function(f) strsplit(basename(f), split = '.', fixed = TRUE)[[1]][1])) %in% tolower(singleType)
file_names = file_names[keep]
info = file.info(file_names) # remove empty files
file_names = rownames(info[info$size > 0, ])
...
dp$tumor[dp$tumor == 'Breast-LobularCa'] = 'Breast-LobularCA'
dp$tumor[dp$tumor == 'Eso-AdenoCa'] = 'Eso-AdenoCA'
write.table(dp, './lncrna.ncrna/sig.only/driverpower.lncrna.ncrna.eigen70.singleTumor.tsv', sep = '\t', quote = F, row.names = F)
# combined single
combined.single = combined[combined$tumor %in% singleType, ]
combined.single = rbind(combined.single, dp)
dat.single = as.data.frame(table(combined.single$tumor, combined.single$method))
