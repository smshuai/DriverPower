...
library(ggplot2)
library(gplots)
source('~/DriverPower/figures/ggplot_theme.R')
res = dcast(mut, donor_ID ~ element_ID)
...
melt.data<-melt(alb, id.vars="donor_ID")



dat = t(as.matrix(alb))
heatmap.2(dat, density.info="none", trace="none", Colv=F, Rowv = F, col=c('white', 'red'), key=F,
          adjRow = c(0,NA), colsep=1:ncol(dat), sepcolor = 'grey', margins = c(5, 10))

row.names(dat) = c('CDS', 'SS', '3UTR', 'Promoter', 'CN LOSS', 'CN GAIN', 'SV')
...
# RNA-seq
rnameta = read.table('./rnaseq_metadata_lite.txt', header=T, sep='\t', stringsAsFactors = F)
rnameta = rnameta[rnameta$histology_abbreviation=='Liver-HCC',]
rnaseq = read.table('./tophat_star_fpkm_uq.v2_aliquot_gl.tsv', header=T, sep='\t', check.names = F)
...
alb.expression = merge(alb.expression, rnameta[,c(1,2,4)])
...
ggplot(alb.expression, aes(x=is_tumour, y=FPKM.UQ, fill=is_tumour)) + geom_boxplot() + ylab('Expression (FPKM-UQ) of ALB') +
...
ggplot(alb, aes(x=as.factor(SV), y=FPKM.UQ)) + geom_boxplot()
...
utr3 = cbind(data.frame(rna=alb[alb$`3utr`==1, 9]), name='3UTR', stringsAsFactors=F)
...
# huge boxplot
dat.rna$name = factor(dat.rna$name, levels=c("Normal","WT","CDS","Promoter","SS","3UTR", 'CN LOSS', 'CN GAIN', 'SV'))
xlabs <- paste0(levels(dat.rna$name),"\n(N=",table(dat.rna$name),")")
ggplot(dat.rna, aes(x=name, y=rna, fill=name)) + geom_boxplot() + ylab('Expression (FPKM-UQ) of ALB') +
  theme_Publication() + scale_x_discrete(labels=xlabs) + theme(axis.title.x = element_blank()) +
