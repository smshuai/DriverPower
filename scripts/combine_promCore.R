...
# LARVA
larva = read.table('LARVA.promCore.tsv', header=T, stringsAsFactors = F)
# ncDriver
ncd = read.table('ncDriver.promCore.tsv', header=T, stringsAsFactors = F)

combined = rbind(ad2, cd.res, nbr, mutsig, ncdd, rd, odf, larva, ncd)
...
ggplot(dat.origin, aes(x=Var2, y=Var1, fill=Freq, label=Freq)) + geom_tile() + geom_text(color='white') + theme_Publication() +
  theme(legend.title = element_blank(), 
        ...
        axis.text.x = element_text(angle = 45, hjust = 1)) +
  guides(fill=F)
ggsave('../../../figures/plot/num.sig.metaOrigin.promCore.heatmap.png')
...
ggsave('../../../figures/plot/nsup.promCore.metaOrigin.barplot.png')
