...
bed = pd.read_table(sys.argv[2], sep='\t', header=None) # 4th column should be binID
...
idx.sort()
binID.sort()
assert np.array_equal(idx, binID), 'ERROR | binID in cg table does not match binID in BED'
