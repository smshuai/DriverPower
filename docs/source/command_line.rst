...
.. important::
    ...
    Model files can be moved to another directory as long as ``--modelDir`` is specified in ``infer``.
...
* **Parameters**
    ...
    * ``--gbmParam``: [*optional*] path to the parameter pickle for GBM.
      The pickle file must contain a valid python dictionary for
      `XGBoost parameters <https://github.com/dmlc/xgboost/blob/master/doc/parameter.md>`_.
...
* **Notes**
    ...
    * Input data requirements can be found at :doc:`data format <data_format>`.
